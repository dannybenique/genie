<?php
  ...
  $detalle = "";
  $importeLetras = "";
  $sql = "select d.*,mv.nombre as tipo_mov,upper(fn_get_letras(importe)) as importe_letras1,upper(' con '||round(importe-trunc(importe),2)) as importe_letras2 from bn_movim_det d join sis_mov mv on d.id_tipo_mov=mv.id where id_movim=:movimID order by item";
  $qry = $db->query_all($sql,$params);
  if($qry) {
    foreach($qry as $rs){
      $detalle .= '<tr><td style="text-align:left;">'.$rs["tipo_mov"].'</td><td style="text-align:right;">'.number_format($rs["importe"],2,".",",").'</td></tr>';
      $importeLetras = trim($rs["importe_letras1"]).$rs["importe_letras2"];
    }
  }
