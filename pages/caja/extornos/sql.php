<?php
  ...
  function reg_movim_det($tipo_movID,$prestamoID,$movimID,$productoID,$importe){
    ...
    $detalleID = $fn->getValorCampo("select coalesce(max(id)+1,1) as code from bn_movim_det;", "code");
    $item = $fn->getValorCampo("select coalesce(max(item)+1,1) as item from bn_movim_det where id_movim=".$movimID, "item");
