<?php
  ...
  switch ($data->TipoQuery) {
    ...
    case "pagos_cambioMontoBloque"://cambia el importe de los registros en bloque
      ...
      $sql = "update app_colprod set importe=:importe,sys_ip=:sysIP,sys_user=:userID,sys_fecha=now() where bloqueo=0 and id_colegio=:colegioID;";
      $params = [
        "importe" => $data->importe,
        ":colegioID"=>$web->colegioID,
        ":sysIP"=>$fn->getClientIP(),
    ...
    case "pagos_cambioVencimientoBloque"://cambia el año de vencimiento de los registros seleccionados
      //actualizamos todos los registros
      $sql = "update app_colprod set vencimiento = TO_DATE(:yyyy || '-' || extract(month from vencimiento) || '-' || extract(day from vencimiento),'YYYY-MM-DD'),sys_ip=:sysIP,sys_user=:userID,sys_fecha=now() where bloqueo=0 and id_colegio=:colegioID;";
      $params = [
        ":yyyy" => $data->yyyy,
        ":colegioID"=>$web->colegioID,
        ":sysIP"=>$fn->getClientIP(),
